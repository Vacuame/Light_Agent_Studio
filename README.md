...
定位规则：
1. 优先匹配顶层任务目录名和其中的 overview.md。
2. 进入匹配任务后，沿目录树查找与你的角色、职责、父子关系、上游 handoff 对应的工作区。
3. 如果已有唯一匹配工作区，接手该工作区。
4. 如果顶层任务明确但没有匹配工作区，在合适父级下创建自己的 Agent 工作区；父级不明确时先问我。
5. 如果有多个候选任务或工作区，列出候选项和判断依据，不要擅自选择。
6. 如果没有匹配顶层任务，询问我是否创建新任务目录；不要自行创建顶层任务。
7. 不要要求我手动输入任务目录路径，除非目录树和任务描述都无法区分。
...
state/tasks/
  用户明确开启的顶层任务目录；每个任务包含 overview.md 和嵌套 Agent 工作区
...
5. 不清楚就问，不允许用猜测替代决策。
6. 临时协作信息、交接和回报写到 `state/tasks/<task-id>/` 下的对应 Agent 工作区。
7. 已确认的长期事实写到 `docs/`。
8. `overview.md` 是顶层任务总览，只写跨角色稳定共识；`handoff.md` 是工作区交接；`report.md` 是工作区回报。
9. 关键交接、回报或关闭前必须过对应质量门。
