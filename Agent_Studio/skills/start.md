...
2. 按该角色文件的 `## 角色专属资源`，结合当前任务需要选择读取相关 skill、质量门、状态层文件和正式产物。
3. 如果是组合角色，合并所有参与角色的角色专属资源，按当前任务需要去重读取；权限冲突时采用更严格权限。
...
如果需要创建工作区，先说明建议路径、层级关系和原因，等用户确认。无法唯一判断时，列出候选并询问用户，不要自行创建。
