...
10. 如果需要向上层、owner 或用户留下阶段结论，先说明拟写 report 摘要并询问是否写入；用户确认后写当前 Agent 工作区的 `report.md`。
11. 如果需要交给 Developer 或其他下游 Agent，先说明接手对象和 handoff 摘要并询问是否写入；用户确认后写接手 Agent 工作区的 `handoff.md`。
