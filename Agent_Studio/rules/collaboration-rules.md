...
handoff 是上游基于当时认知整理的交接输入，不是必须逐字执行的指令。上游通常不深入实现细节，其中的事实断言和实现落点可能与真实代码或环境不符。接手 Agent 对 handoff 内容按三类处理：

- 事实断言（代码位置、环境形态、机制行为、调用关系）：默认待核实。影响实现或测试方案的关键事实，必须对照真实代码和环境验证后才能作为依据；发现不符时以事实为准，反馈上游或用户，不得按原文执行。
- 实现落点与方案：是上游建议，不是唯一解。接手者必须核实落点与既有系统机制的兼容性——同类内容现在如何运作、本次改动从写入到生效会经过哪些既有步骤；发现更符合既有系统运作方式的落点时，向用户提出，确认后采用。
- 边界约束（架构边界、模块边界、层级约束、"不做什么"清单）：仍然有效。发现与现实冲突时反馈用户确认，不得自行变更边界。

偏离 handoff 的落点或方案不需要豁免，但必须在交付说明中记录偏离内容和原因，让上游和用户知道实际发生了什么。上游写作义务不降级：无法亲自核实的细节必须写进"临时假设"，不得写成"已确认内容"。
...
“需要读取哪些文件”只列与当前交接任务直接相关的任务上下文、上游交接、正式产物、源码、测试记录或状态文件。

不要把通用启动文件、通用协作规则、当前角色文件、当前 skill、质量门或项目配置逐条写入 `handoff.md` 的必读文件；这些由接手 Agent 按 rules、角色专属资源和当前任务自行读取。

例外：如果交接任务本身就是修改 Agent Studio 规则、角色、skill、质量门或项目配置，可以把相关框架文件列为任务必读文件。

如果交接给 Developer，还必须写清楚实现落点摘要。落点涉及的交互细节（链路行为、既有机制）无法亲自核实时，归入临时假设或标注核实状态，不得写成已确认事实。
...
如果模块设计或交接中的实现落点与真实代码冲突，Developer 必须反馈给 Module Designer 或用户确认，不得自行改变模块边界或代码层级。
